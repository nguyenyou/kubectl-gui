...
export async function kubeDescribePods(name: string) {
  const { stdout } = await new Command("kubectl-describe-pod", [
    "describe",
    "pods",
    name,
  ]).execute();
...
export async function kubeGetPodYaml(name: string) {
  const { stdout } = await new Command("kubectl-get-pod-yaml", [
    "get",
    "pods",
    name,
    "-o",
...
export async function kubeGetLogs(name: string) {
  const { stdout } = await new Command("kubectl-get-logs", [
    "logs",
    name,
    "--tail",
    "200",
...
export async function getPodsInfo() {
